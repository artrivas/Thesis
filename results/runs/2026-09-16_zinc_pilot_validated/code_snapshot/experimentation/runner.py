...
import math
import hashlib
from experimentation.artifacts import (SCHEMA_VERSION, pair_records, representation_details,
    persist_row_artifacts, row_artifacts_valid, read_artifact, write_artifact)
...
def run_experiment(
    ...
    *,
    ...
    if resume:
        ...
        old_manifest = config.outputs.root / "run_manifest.json"
        if old_manifest.exists() and result_path.exists():
            previous_code = json.loads(old_manifest.read_text()).get("code_fingerprint")
            if previous_code and previous_code != _code_fingerprint():
                raise ValueError("Experiment code changed; use a new run directory")
        _validate_protocol_and_repair(result_path, config, workflow_instances)
    ...
    rows_written = 0
    run_code_fingerprint = _code_fingerprint()
    with result_path.open("a" if append else "w", newline="", encoding="utf-8") as handle:
        ...
        def persist(row: dict[str, object]) -> None:
            ...
            row["code_fingerprint"] = run_code_fingerprint
            persist_row_artifacts(row, result_path.parent)
...
def workflows_from_config(config: ExperimentConfig) -> list[Workflow]:
    ...
    if any(dc.family == "zinc" for dc in config.resolved_dataset_configs()):
        from experimentation.workflows import WLSubtreeMMDWorkflow
        available["wl_subtree_kernel_mmd"] = WLSubtreeMMDWorkflow(label_initialization="degree")
...
def merge_runs(shard_paths: list[Path | str], output_path: Path | str) -> Path:
    ...
    sources = [(_shard_result_csv(Path(shard))) for shard in shard_paths]
    all_rows = [(source, row) for source in sources for row in read_result_rows(source)]
    protocols = {(row.get("schema_version", ""), row.get("randomness_protocol", "")) for _, row in all_rows}
    if len(protocols) > 1:
        raise ValueError("Cannot merge incompatible randomness/schema protocols")
    code_versions = {row.get("code_fingerprint", "") for _, row in all_rows}
    if len(code_versions) > 1:
        raise ValueError("Cannot merge different experiment code versions")
    destination = Path(output_path).parent
    dataset_configs = {}
    for _, row in all_rows:
        if row.get("schema_version") == SCHEMA_VERSION:
            params = json.loads(row["dataset_params"])
            params.pop("seed", None)
            signature = json.dumps(params, sort_keys=True)
            if row["dataset"] in dataset_configs and dataset_configs[row["dataset"]] != signature:
                raise ValueError("Cannot merge different dataset or master-seed configurations")
            dataset_configs[row["dataset"]] = signature
    for source, row in all_rows:
        if row.get("schema_version") == SCHEMA_VERSION and row["status"] != "failed":
            if not row_artifacts_valid(row, source.parent):
                raise ValueError("Cannot merge missing/corrupt graph artifacts")
        for field, checksum_field in (("graph_artifact", "graph_sha256"), ("workflow_artifact", "workflow_sha256")):
            if row.get(field):
                value = read_artifact(source.parent, row[field], row[checksum_field])
                name = Path(row[field]).name.removesuffix(".json.gz")
                row[field], row[checksum_field] = write_artifact(destination, name, value)
        key = _row_key(row)
        if key not in seen:
            seen.add(key)
            merged.append(row)
...
def _validate_protocol_and_repair(path, config, workflows):
    if not path.exists() or not path.stat().st_size:
        return
    rows = read_result_rows(path)
    configurations = {json.dumps(asdict(replace(dc, seed=0)), sort_keys=True)
                      for dc in config.resolved_dataset_configs()}
    methods = {w.name: json.dumps(w.parameters(), sort_keys=True) for w in workflows}
    current_code = _code_fingerprint()
    repaired = []
    for row in rows:
        if row.get("schema_version") != SCHEMA_VERSION:
            raise ValueError("Incompatible historical/schema protocol; use a new run directory")
        if row.get("code_fingerprint") != current_code:
            raise ValueError("Experiment code changed; use a new run directory")
        params = json.loads(row["dataset_params"])
        if params.get("randomness_protocol") != row.get("randomness_protocol"):
            raise ValueError("Randomness protocol metadata mismatch")
        params["seed"] = 0
        if json.dumps(params, sort_keys=True) not in configurations:
            raise ValueError("Dataset/randomness configuration changed; use a new run directory")
        if row["workflow"] in methods and _canonical_jsonish(row["workflow_params"]) != methods[row["workflow"]]:
            raise ValueError("Workflow configuration changed; use a new run directory")
        # Failed cell construction has no graph data; rerun-failed controls retries.
        if row["status"] == "failed" or row_artifacts_valid(row, path.parent):
            repaired.append(row)
    if len(repaired) != len(rows):
        temporary = path.with_suffix(".repair.tmp")
        write_result_rows(repaired, temporary)
...
def _code_fingerprint():
    content = {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in _code_files()}
...
def write_run_manifest(
    ...
    *,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    snapshot = config.outputs.root / "code_snapshot" / "experimentation"
    snapshot.mkdir(parents=True, exist_ok=True)
    code_fingerprint = _code_fingerprint()
    if status == "running" or not path.exists():
        for source in _code_files():
            (snapshot / source.name).write_bytes(source.read_bytes())
    else:
        code_fingerprint = json.loads(path.read_text()).get("code_fingerprint", code_fingerprint)
    payload = {
        "schema_version": SCHEMA_VERSION,
        "code_fingerprint": code_fingerprint,
        "code_changed_during_run": code_fingerprint != _code_fingerprint(),
        "acceleration": acceleration_summary(),
        "workflow_parameters": {w.name: w.parameters() for w in workflows},
        "run_id": run_id,
        "status": status,
...
def _run_workflow_grid(
    ...
    graph_records = pair_records(paired)
    ...
    for workflow in workflows:
        ...
        details = representation_details(workflow, result.pop("_representations", None))
        if result["status"] == "success" and details["available"]:
            distances = details["paired_distances"]
            if not math.isclose(sum(distances)/len(distances), result["paired_score"], rel_tol=1e-10, abs_tol=1e-10):
                raise ValueError("Graph distances do not reconstruct paired score")
            if "mmd_contributions" in details:
                contributions = details["mmd_contributions"]
                if not math.isclose(sum(contributions)/len(contributions), result["distribution_score"], rel_tol=1e-10, abs_tol=1e-10):
                    raise ValueError("MMD contributions do not reconstruct distribution score")
...
def _run_workflow_with_memory(workflow: Workflow, paired: PairedDistribution) -> dict[str, object]:
    ...
    result = workflow.run_with_details(paired.original_graphs, paired.perturbed_graphs)
