...
- Results **never** go into the source tree. `results/runs/` sits at the repo root,
  beside `src/`, and every command runs from the repo root. An arbitrary flat
  output directory still works via `--output-root`, but new runs default to the
  traceable `results/runs/` tree; the pre-traceability `outputs/` trees are
  archived under `archive/legacy-outputs/`.
- Which run is current, and which of its artifacts are citable, is recorded in
