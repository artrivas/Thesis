...
```text
results/runs/<run-id>/results/results.csv
```
...
```text
python3 -m experimentation.cli run --config debug
python3 -m experimentation.cli run --config full
python3 -m experimentation.cli evaluate_results --results results/runs/<run-id>/results/results.csv
python3 -m experimentation.cli generate_figures --results results/runs/<run-id>/results/results.csv
...
```text
python3 -m experimentation.cli run --config full --device cuda --console-log
```
...
```text
python3 -m experimentation.cli run --config full --device cuda --no-resume --console-log
```
...
```text
python3 -m experimentation.cli run --config full --device cuda --rerun-failed
```
...
Use `--workflow` to run one or more workflow families in a separate run directory.
This is one way to parallelize full runs across terminals without multiple
processes writing the same CSV; each invocation gets its own `results/runs/<id>/`:

```text
python3 -m experimentation.cli run --config full --workflow structural_statistics_mmd --no-resume --console-log
python3 -m experimentation.cli run --config full --workflow wl_subtree_kernel_mmd --no-resume --console-log
python3 -m experimentation.cli run --config full --workflow native_netlsd --no-resume --console-log
python3 -m experimentation.cli run --config full --workflow diversity_curves_shortest_path --no-resume --console-log
```

Combine the per-workflow runs afterwards with `experimentation.cli merge`, the same
way seed-range shards are combined — see [`parallelism.md`](parallelism.md).
