...
```bash
python3 -m experimentation.cli run --config full --console-log
```
...
`run_debug_experiment` and `run_full_synthetic_experiment` predate the traceable
run directories. They still exist and write into a single flat output root
(default `results/legacy/debug` and `results/legacy/full_synthetic`), but they
produce no `run_manifest.json`, so results from them are not traceable:

```bash
python3 -m experimentation.cli run_debug_experiment --output-root results/legacy/debug
python3 -m experimentation.cli run_full_synthetic_experiment --output-root results/legacy/full_synthetic
```

Prefer `run --config …` for anything that will be cited.
