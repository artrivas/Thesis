...
## Item 9 — Repository reorganization

**Modules:** `config.py`, `cli.py`, `runner.py`, `dashboard.py`, `scripts/*`, all docs.

- **Layout flattened.** `docs/`, `tests/`, `scripts/`, `data/` and `results/` moved
  from `src/` to the repo root; `src/` now holds only the `experimentation` package.
  All commands run from the repo root. Added `pyproject.toml` (src-layout package,
  `pytest` `pythonpath=src`, `figures`/`dashboard` extras, `experimentation` console
  script) and a root `README.md`.
- **Fixed a silently ineffective `.gitignore`.** The results policy lived in
  `results/.gitignore` at the repo root, but runs were written to `src/results/runs/`
  because paths are relative and commands ran from `src/`. The policy therefore
  never applied and every bulky artifact was being committed, including a 21 MB
  result CSV. There is now a single root `.gitignore` covering the real paths.
- **Merged run promoted to source of truth.** `results/runs/2026-07-03_merged/`
  (20,328 rows; the full synthetic grid plus IMDB-BINARY) was untracked, along with
  the four scripts that generate the thesis and poster figures from it. Its result
  table is now committed gzipped (23 MB → 1.1 MB) together with its evaluation
  tables, `figures_final/` and `poster_figures/`. It also gained a
  `run_manifest.json` recording its two shards and their git commits, which a
  merged run previously did not get.
- **Gzip-transparent reads.** `resolve_result_csv()` falls back to a sibling
  `results.csv.gz` when the plain CSV is absent, and `read_result_rows()`,
  `_shard_result_csv()` and the four figure scripts go through it. A fresh clone can
  regenerate every figure without re-running the experiments.
- **Superseded material archived, not deleted.** The six pre-traceability
  `outputs/` trees, two aborted zero-row runs, two stray `src/*.log` files and a
  scratch notebook moved to the untracked `archive/`, documented in
  `archive/README.md`. Legacy output-root defaults were retargeted from `outputs/…`
  to `results/legacy/…` so nothing recreates the archived tree.
- **`results/RESULTS.md`** added: which run is current, what to cite, what is
  diagnostic-only. `fuentes/` and `papers/` merged into `references/` (two
  byte-identical duplicates dropped); the thesis draft moved to
  `docs/PFC-Dimael-draft.pdf`.
- Historical audit and log documents keep their original paths and carry a
  banner saying so. All 135 tests pass from the new root.
